...
def _save_output(content: str, filename: str = "magma_output.txt") -> None:
...
def _get_key() -> bytes | None:
    ...
    if choice == "1":
        ...
        try:
            ...
            if len(key) != 32:
                print("  [Error] Magma key must be exactly 32 bytes (64 hex chars).")
                return None
            return key
        except ValueError:
...
def generate_key() -> None:
    if not _check_library():
        return
    print("\n--- Magma Key Generation (256-bit / 32 bytes) ---")
    key = get_random_bytes(32)
    hex_key = key.hex()
    print(f"  Key (hex): {hex_key}")
    save = input("\n  Save key to file? (y/n): ").strip().lower()
    if save == "y":
        _save_output(f"Magma Key (256-bit):\n{hex_key}\n", "magma_key.txt")


def encrypt_message() -> None:
    if not _check_library():
        return
    print("\n--- Magma-CBC Encryption ---")
    key = _get_key()
    if key is None:
        return
    ...
    try:
        ...
        if save == "y":
            output = (
                f"Magma CBC Encryption Output\n"
                f"Algorithm : GOST R 34.12-2015 Magma\n"
                f"Key       : {key.hex()}\n"
                f"IV        : {hex_iv}\n"
...
def magma_menu() -> None:
    while True:
        ...
        print("  Standard : GOST R 34.12-2015 / GOST 28147-89")
        print("  Mode     : CBC")
        print("  Block    : 64-bit (8 bytes)")
